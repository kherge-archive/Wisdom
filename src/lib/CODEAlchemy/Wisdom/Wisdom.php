<?php
    ...
    class Wisdom
    {
        ...
        {
            if ((null !== $values) && (false === is_array($values)) && ! ($values instanceof ArrayAccess))
            {
                throw new InvalidArgumentException('The value of $values is not an array or an instance of ArrayAccess.');
            }
            ...
            {
                $cache = new ConfigCache($this->cache . DIRECTORY_SEPARATOR . $file . '.php', $this->debug);
            ...
            {
                throw new InvalidArgumentException(sprintf('No loader available for file: %s', $file));
            }
            ...
            if (isset($cache))
            {
                $cache->write('<?php return ' . var_export($data, true) . ';', array(new FileResource($found)));
            }
